...
function ale_enqueue_styles() {
    ...
    wp_register_style( 'orange', THEME_URL . '/css/colors/orange.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'green', THEME_URL . '/css/colors/green.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'black', THEME_URL . '/css/colors/black.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'red', THEME_URL . '/css/colors/red.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'blue', THEME_URL . '/css/colors/blue.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'purple', THEME_URL . '/css/colors/purple.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'sea', THEME_URL . '/css/colors/sea.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'nephritis', THEME_URL . '/css/colors/nephritis.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'asbestos', THEME_URL . '/css/colors/asbestos.css', array(), ALETHEME_THEME_VERSION, 'all');

    //Check if User have styles cookie, if no show the color from Theme Options.
    if(ale_get_option('site_color')){
        if(!isset($_COOKIE["sochacolor"])) {
            wp_enqueue_style(ale_get_option('site_color'));
        } else {
            wp_enqueue_style($_COOKIE["sochacolor"]);
        }
    } else {
...
function ale_customcss(){
    ale_part('css-option');
    if(ale_get_option('customcsscode')){ echo '<style type="text/css">'.str_replace('&gt;','>',ale_get_option('customcsscode')).'</style>';}
}
...
function ale_enqueue_scripts() {
    ...
    wp_register_script( 'jquery.easypiechart.min', THEME_URL . '/js/libs/jquery.easypiechart.min.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'jquery.cookie', THEME_URL . '/js/libs/jquery.cookie.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'jquery.hoverdir', THEME_URL . '/js/libs/jquery.hoverdir.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );

    //Get Scrollable on Inspiration Single Post.
    ...
    if(is_page_template('template-home-one.php')){
        wp_enqueue_script( 'jquery.nivo.slider.pack' );
    }

    if(is_page_template('template-about-three.php')){
        wp_enqueue_script( 'jquery.easypiechart.min' );
    }

    //Get Post Like on Single pages
    if(is_single()){
        wp_enqueue_script('like_post', get_template_directory_uri().'/js/libs/post-like.js', array('jquery'), '1.0', 1 );
...
function ale_footer() {
	...
	<?php if(is_single()){ if(ale_get_option('comments_style')=='fb' or ale_get_option('presscomments_style')=='fb' or ale_get_option('gallerycomments_style')=='fb' or ale_get_option('videocomments_style')=='fb' or ale_get_option('inspirationcomments_style')=='fb'){ ?>
    <div id="fb-root"></div>
